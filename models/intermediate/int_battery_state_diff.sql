with diff_cte as (
    ...
    from
        {{ ref('int_battery_state_clean').render() }}
    where
        battery_percent is not null
        and {{ microbatch_condition(ref('int_battery_state_clean'), end_offset_days=1) }}
)
...
from
    diff_cte
where
    {{ microbatch_condition(ref('int_battery_state_clean')) }}
